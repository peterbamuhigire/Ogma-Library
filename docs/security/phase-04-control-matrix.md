...
| CTRL-OGMA-004..007 | Untrusted PDF worker isolation. | Pass with platform notes | `phase-05-pdf-worker-isolation.md`; `PdfWorkerIsolationTests`; production DI uses `IsolatedPdfRendererFactory`; assets copy only completed worker-sandbox outputs. | Phase 16/20 release gates must benchmark subprocess render impact on reference hardware. |
...
| P04-R1 / CTRL-OGMA-004..007 | Phase 05 routes production PDF render/text/asset work through `OgmaLibrary.Workers` subprocesses with sandboxed temp/output handoff and fault-injection tests for network, process, temp-escape, malformed PDF, and valid render paths. |
