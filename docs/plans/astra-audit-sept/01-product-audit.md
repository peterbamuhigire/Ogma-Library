...
Fresh Windows verification passed 1,160 automated tests (955 core, 42 architecture, 163 UI), locked restore, Release build, analyzer checks and the shelf3d gates. Requirement accountability failed because its canonical, non-refreshed SRS path is absent; format verification failed on existing diagnostics. See the [exact commands and logs](04-test-report.md). Resolve the source-authority/path discrepancy in phase 01 and the formatting gate in a separately reviewed phase 20 change. Neither failure was hidden by substituting a document or reformatting application source during the audit.
