...
| --- | --- | --- | --- | --- | ---: | --- |
| P05-T1 | F-SEC-001, F-SEC-002; audit report `docs/analysis-report-2026-07-07` | `docs/analysis-report-2026-07-07/*`; `src/OgmaLibrary.Workers; src/OgmaLibrary.Reader; src/OgmaLibrary.Infrastructure/Assets; tests/OgmaLibrary.Tests/Security` | Re-read the audit finding rows and the relevant reference docs. Confirm no prerequisite finding is still open. Record any impossible instruction through the Deviation Protocol. | implementation-status-auditor | 0.25 d | Prevents silent rescoping. |
| P05-T2 | F-SEC-001, F-SEC-002 | `src/OgmaLibrary.Workers; src/OgmaLibrary.Reader; src/OgmaLibrary.Infrastructure/Assets; tests/OgmaLibrary.Tests/Security` | Implement the smallest production change that directly resolves the finding set. Preserve existing architecture boundaries and public contracts unless the referenced ADR/SRS explicitly requires a change. | web-app-security-audit; code-safety-scanner; system-architecture-design | 1.0-2.0 d | Main implementation risk; do not touch unrelated modules. |
