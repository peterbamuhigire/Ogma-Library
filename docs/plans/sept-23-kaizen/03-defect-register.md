...
| K06 | P2 | MEASURED | The Release output ships `Avalonia.Diagnostics.dll` and native runtime folders for about 40 RIDs (android, ios, linux-musl, s390x and others). `Avalonia.Diagnostics` is conditioned to Debug in `OgmaLibrary.App.csproj:29` yet its DLL is in the Release output. Root cause (MEASURED 2026-09-25): `packages.lock.json` is produced by a Debug-configuration restore and CI runs `dotnet restore --locked-mode` without a configuration before `build -c Release --no-restore`, so Release inherits the Debug-only package; a Release restore rewrites the lock file. Phase 26 must make restore configuration-independent (for example an MSBuild-level exclusion of the diagnostics assets in Release). There are about 77 untracked `src/*/tmp/` build directories. | `ls src/OgmaLibrary.App/bin/Release/net10.0/runtimes` | 00, 26 |
...
| K96 | P2 | MEASURED | Timing-budget tests are load-sensitive: `PdfWorkerSessionStabilityTests.IsolatedPdfRenderer_WorkerKilledMidRead_NextPageRendersWithinTwoSeconds` failed once in the fast suite while two parallel lanes loaded the 4-core machine, then passed 5/5 in isolation. Budget assertions in the per-commit suite need load-tolerant margins or a move to `Category=Benchmark`. | Fast-suite run 2026-09-25 | 24 |
...
| K93 | P2 | MEASURED | (Found by the Phase 01 harness, 2026-09-25.) After a folder is chosen, `ShelfSidebarViewModel` changes `IsLoading`, `CanCreateShelf`, `CanDeleteSelectedShelf` and `CanRenameSelectedShelf` off the UI thread: 32 `ui.thread.violation` log events in one G2 run. `MainShellViewModel.ChooseFolderAsync` runs `ShelfSidebar.LoadAsync(...).ConfigureAwait(false)` inside `Task.Run` (K72 class, missed by Phase 02). | G2 app log (`ui.thread.violation`) | 06 |
| K94 | P1 | MEASURED | (Phase 01 harness.) Next-page presses are silently ignored while a page turn is still in flight, and the reader then stalls: in G3 each turn took 0.4–0.8 s to update the page number, 4 presses were ignored, and page 43 → 44 never advanced within 10 s. | G3 timings (`pageTurn.*`) | 04, 12 |
| K95 | P1 | MEASURED | (Phase 01 harness.) A corrupt `library-settings.json` is logged at Warning or above, but the user sees no message: no degraded panel and no toast within 20 s, and the shell shows the normal first-run state. | G8 corrupt-settings timings and log | 05 |
