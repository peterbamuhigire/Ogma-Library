...
| Windows/macOS CI matrix | `.github/workflows/ci.yml`; Phase 09 WP9-T6 cross-platform format/build/test gate is wired for `main`/`develop` pushes and PRs on `windows-latest` and `macos-latest` | Workflow configured; latest remote run status not fetched in this environment |
