...
| `dotnet test tests/OgmaLibrary.Tests/OgmaLibrary.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~LanHostEndpointTests` | Passed: 1 real HTTPS Host endpoint flow, including immediate 401 rejection of an already-issued managed-profile session after profile revocation |
