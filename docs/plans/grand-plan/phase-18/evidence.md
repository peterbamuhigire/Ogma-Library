...
| `dotnet test tests\OgmaLibrary.Tests\OgmaLibrary.Tests.csproj --configuration Release --no-build --filter "FullyQualifiedName~SchoolUsageDashboardServiceTests" --logger "console;verbosity=minimal"` | Passed: 4 dashboard tests covering ten-query multi-day aggregation, tokens, costs, quota percentage, date filtering, zero-usage enrolled profiles, and invalid range rejection |
...
| `dotnet test tests/OgmaLibrary.Tests/OgmaLibrary.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~LanHostEndpointTests` | Passed: 1 real HTTPS Host endpoint flow, including assigned-role resolution for a managed enrollment and immediate 401 rejection of its already-issued session after profile revocation |
| `dotnet test tests/OgmaLibrary.Tests/OgmaLibrary.Tests.csproj --configuration Release --no-restore --filter FullyQualifiedName~SchoolProfileEnrollmentServiceTests` | Passed: 6 enrollment tests, including matching-session revocation with timestamp consistency and unrelated-session preservation |
