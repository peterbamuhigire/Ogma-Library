# Phase 17 — Client / Classroom Mode & Multi-User
...
profile, and immediately browses and reads the school library — with their own
...
| **ADRs introduced** | ADR-0011 (proposed — see §7) |
...
1. An Ogma installation can switch to Client mode: it discovers a Host via mDNS
   ...
   token — all within a single onboarding UI flow.
...
6. The standalone product (Standalone mode, local library) is byte-for-byte
   ...
   catalogue.
7. ADR-0011 is authored, recording the identity model, role taxonomy, private-
...
- Profiles: `student`, `teacher`, `guest`. Profile is a local construct (a row
  ...
  18 admin enrollment — stub in this phase via a self-issued identity for Phase
  17 testing).
...
- Offline cache: LRU cache of catalogue projections and rendered page images
  ...
  conditional refresh. On LAN drop: show "Offline — reading from cache" chip;
...
- ADR-0011: identity, roles, private-state storage, sync strategy.
...
- Phase 17 does not change or extend the Host — Host endpoints from Phase 16 are
  consumed as-is.
- Cloud sync (OQ-08 — schema-ready but not wired).
- Linux.
...
| --- | --- | --- | --- |
| FR-CLIENT-001 | V2 | Mode switcher: Standalone ↔ Connect to Host | Integration test: mode toggle persists across restart; correct DB is loaded per mode |
...
| FR-CLIENT-008 | V2 | Offline cache: catalogue + rendered pages; reader works on LAN drop | Fault-injection test: LAN disconnected mid-session → reader continues from cache |
...
| FR-CLIENT-010 | V2 | Optional sync: private state pushed to Host under student identity | Integration test: sync → Host stores blob by profileId; restore on new device |
| FR-CLIENT-011 | V2 | Conflict surfacing: last-write-wins + student chooses on collision | Integration test: conflicting annotation timestamps → conflict dialog → student choice persists |
| FR-CLIENT-012 | V2 | Guest profile: browse + read, no persistent private state | Integration test: guest session → no DB row written; next guest session starts clean |
| FR-CLIENT-013 | V2 | Standalone mode unchanged: no regression on any Phase 00–15 flow | Golden-corpus regression suite passes with mode = Standalone |
| NFR-CLIENT-001 | V2 | Catalogue load from Host ≤ 2 s P95 (2,000 books, warm LAN) | Load test: 2,000-book projection, P95 ≤ 2 s |
| NFR-CLIENT-002 | V2 | Offline cache hit: page render ≤ 100 ms P95 (NFR-OGMA-005) | Performance test: 10 cache-hit page requests, P95 ≤ 100 ms |
...
| ADR-0011 | V2 | Identity, roles, private state, sync strategy documented | ADR-0011 authored and owner-ratified |
...
- **Phase 03**: design system, design tokens, icon system — profile switcher and
...
1. **Identity:** a `ProfileId` (UUID v4, generated locally on first enrollment)
   ...
   stores the student's reading content — only the profile metadata (display name,
   role) and, if the student opts into sync, an opaque compressed blob of the
   student's private state.

2. **Roles:** `student` (default) — browse/read/annotate privately; `teacher` —
   browse/read/annotate + initiate shelf curation (Phase 18 for curation approval);
   `guest` — browse/read, no persistent state, no sync. Role is embedded in the
...
4. **Offline cache:** LRU on-disk cache at `<sidecar>/classroom/cache/`; keyed
   ...
   500 MB). Cache is not private-state — it contains Host-served rendered images,
...
Interfaces consumed:
- `ILibraryHostClient` (new) — typed HTTP client wrapping Phase 16 Host API.
- `ICredentialStore` — session token storage (CTRL-OGMA-001).
- `IOfflineCacheService` — LRU page/catalogue cache.
- `IStudentPrivateRepository` — CRUD on the per-student SQLite DB.
- `IMdnsResolver` — discover Host mDNS records.

Interfaces owned:
- `IClassroomModeService` — switch modes; current mode observable.
- `IProfileService` — create/select/delete profiles; role resolution.
- `ISyncService` — trigger sync; conflict resolution callback.
...
| --- | --- | --- |
| **WP1 — ADR-0011 & architecture** | Author ADR-0011; ClassroomClient bounded context scaffold; interfaces; DI wiring; architecture tests | 2 d |
| **WP2 — Host discovery & TOFU client** | mDNS resolver; manual entry; QR join URL parser; certificate TOFU client-side pinning | 3 d |
| **WP3 — Profile management** | Profile create/select/switch; role assignment; guest mode; OS credential store for session token | 2 d |
| **WP4 — Catalogue client view** | Grid/list populated from Host API; sort/filter; book detail; availability status | 3 d |
| **WP5 — Reader integration** | Client reader opens book from Host (page-render + file-stream modes); resume position | 2 d |
| **WP6 — Per-student private DB** | Schema creation; StudentPrivateRepository; annotation/progress/bookmark CRUD | 2 d |
| **WP7 — Offline cache** | LRU cache for catalogue projections + rendered pages; `eTag` conditional refresh; "Offline" chip | 3 d |
| **WP8 — Sync** | Serialize/compress/encrypt private DB blob; `PUT /api/v1/profile/sync`; conflict surfacing UI | 3 d |
| **WP9 — Client mode UI** | Mode switcher; discovery screen; enrollment flow; profile switcher; sync settings | 2 d |
| **WP10 — Testing & CI** | Unit, integration, fault-injection, architecture, performance tests; CI | 3 d |
...
- [x] **Colorful icons + manifest**: `icons.md` defines `ic_connect_to_library`,
  ...
  `ic_offline`, `ic_mode_standalone`, `ic_mode_classroom` — all `⬜ to procure`.
...
- [x] **Performance budgets**: NFR-CLIENT-001 (catalogue load ≤ 2 s),
  NFR-CLIENT-002 (cached page ≤ 100 ms) instrumented; offline cache eviction
  policy tested.
...
- [x] **Documentation**: ADR-0011 authored; XML doc comments on all new public
...
- [ ] ADR-0011 authored and owner-ratified; identity/role/sync decisions recorded.
- [ ] Mode switch (Standalone ↔ Client) persists across app restart; correct DB
...
- [ ] Offline fault-injection test: LAN drop mid-session → reader continues from
      cache for the open book → "Offline" chip visible → reconnect → sync prompt.
...
- [ ] Guest mode: no DB row written during session; app state clean on guest
      logout.
- [ ] Standalone golden-corpus regression: all Phase 00–15 tests pass unchanged.
...
- `mobile-cross:pwa-offline-first` — offline cache design and sync patterns (WP7,
  WP8).
- `security:dual-auth-rbac` / `mobile-rbac` — role-based access control for
  student/teacher/guest (WP3).
- `architecture:event-driven-architecture` — sync event design, conflict
  resolution (WP8).
- `documentation-generation:architecture-decision-records` — ADR-0011 (WP1).
- `frontend-ux:enterprise-ux-process` — multi-user enrollment and profile
  switching flows (WP9).
- `superpowers:test-driven-development` — private DB isolation and fault-injection
  tests (WP6, WP7, WP10).
- `/security-review` — WP2 (TOFU), WP3 (credential store), WP8 (sync blob
  encryption).
...
| Artifact | Location |
| --- | --- |
| ADR-0011 | `docs/architecture/adr-0011-classroom-identity-roles-private-state.md` |
...
| Per-student SQLite schema (in-code, no EF Core migration — separate DB file) | `src/OgmaLibrary.Infrastructure/ClassroomClient/Data/StudentDbContext.cs` |
...
1. **ADR-0011 sign-off**: Peter must ratify the identity and private-state model
   before Phase 17 build begins — this sets the student-data boundary that the
