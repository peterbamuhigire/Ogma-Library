...
| `dotnet test tests\OgmaLibrary.Tests\OgmaLibrary.Tests.csproj --configuration Release --no-build --logger "console;verbosity=minimal" -- xUnit.ParallelizeTestCollections=false xUnit.MaxParallelThreads=1` | Passed: 562 tests |
...
- TLS certificate fingerprint extraction beyond the current Host health
  fingerprint handoff, mDNS picker/QR onboarding polish, sync merge/conflict UI,
