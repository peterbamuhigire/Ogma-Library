...
## Work Package 1 — ADR-0011 & Architecture Scaffold

| ID | Task | Dependencies | Est. | Satisfies |
| --- | --- | --- | --- | --- |
| P17-WP1-T1 | Author `docs/architecture/adr-0011-classroom-identity-roles-private-state.md` covering: ProfileId UUID strategy, role taxonomy, private-state storage path, sync blob format, forward-compatibility with OQ-08 | Phase 16 DoD | 1 d | ADR-0011 |
| P17-WP1-T2 | Owner sign-off on ADR-0011 (Owner ask §14.1) | P17-WP1-T1 | 0 d (gate) | ADR-0011 |
...
| P17-WP1-T6 | Update `SOURCE-SUMMARY.md` §F and §C (new personas: student, teacher, guest, admin) | P17-WP1-T2 | 0.25 d | Documentation |

---

## Work Package 2 — Host Discovery & Certificate TOFU Client
...
## Work Package 3 — Profile Management
...
## Work Package 4 — Catalogue Client View

| ID | Task | Dependencies | Est. | Satisfies |
| --- | --- | --- | --- | --- |
| P17-WP4-T1 | Implement `ILibraryHostClient.GetCataloguePageAsync(page, pageSize, filter, sort)` — typed HTTP client wrapping Phase 16 `/api/v1/catalogue` | P17-WP2-T5 | 0.5 d | FR-CLIENT-006 |
| P17-WP4-T2 | Implement `ILibraryHostClient.GetBookAsync(bookId)` — single book projection | P17-WP4-T1 | 0.25 d | FR-CLIENT-006 |
| P17-WP4-T3 | Adapt existing `CatalogueGridViewModel` and `CatalogueListViewModel` to accept an `ICatalogueSource` abstraction (either local standalone catalogue or Host client) — this is the seam that keeps Standalone unaffected | P17-WP4-T2 | 0.75 d | FR-CLIENT-006, FR-CLIENT-013 |
| P17-WP4-T4 | Availability status: books that are unavailable on Host (file missing on server) shown with `ic_unavailable` chip — use existing availability model from Phase 05 adapted for remote | P17-WP4-T3 | 0.25 d | FR-CLIENT-006 |
...
## Work Package 5 — Reader Integration

| ID | Task | Dependencies | Est. | Satisfies |
| --- | --- | --- | --- | --- |
| P17-WP5-T1 | Implement `ILibraryHostClient.GetPageRenderAsync(bookId, pageNumber, resolution)` — wraps Phase 16 page-render endpoint; caches response in `IOfflineCacheService` | P17-WP4-T1 | 0.5 d | FR-CLIENT-007 |
| P17-WP5-T2 | Implement `ILibraryHostClient.GetFileStreamAsync(bookId)` — wraps Phase 16 file-stream endpoint (fallback when Host is in file-stream mode) | P17-WP5-T1 | 0.25 d | FR-CLIENT-007 |
...
## Work Package 6 — Per-Student Private Database
...
| P17-WP6-T4 | Schema migration within the private DB (managed in-code, not EF Core migration runner — the private DB is student-owned and potentially absent): `StudentDbInitializer.EnsureLatestSchema()` | P17-WP6-T3 | 0.25 d | FR-CLIENT-005, R1 |
...
| P17-WP8-T2 | Upload: `PUT /api/v1/profile/sync` on Host (Phase 16 endpoint stub — add to Phase 16 Host if not present, or note as a Host endpoint extension in this phase); Host stores blob opaquely by profileId | P17-WP8-T1 | 0.5 d | FR-CLIENT-010 |
...
| P17-WP9-T7 | Accessibility walkthrough: keyboard-only navigation of discovery → enrollment → profile selection flow; SR audit | P17-WP9-T6 | 0.25 d | WCAG 2.2 AA, NFR-PROD-008 |

---

## Work Package 10 — Testing & CI
