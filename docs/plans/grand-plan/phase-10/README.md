...
| **Status** | WP1 complete locally; WP2 metadata/search UI implemented locally; WP3 core extraction pipeline implemented locally with generated-PDF smoke coverage; WP4 FTS search implemented locally; WP5 Index Manager backend/UI implemented locally; WP6 keyboard/i18n/a11y/icon placeholders/pseudolocale implemented locally; premium replacement icons, external TOC/scanned golden-corpus fixtures, manual accessibility signoff, and final closeout pending |
...
| 2026-06-01 | WP5 backend started: Index Manager status/rebuild contract, status counts, rebuild events, transactional reset, pipeline-driven rebuild, G7 reliability test, and cancellation recovery added | Codex |
| 2026-06-01 | WP6 UI foundations started: shell-mounted search and Index Manager panels, debounced search view-model with stale-result guard, rebuild confirmation/progress/status UI, Ctrl+F/Ctrl+K/Escape/Enter keyboard paths, en/fr strings, automation names, and UI tests added | Codex |
