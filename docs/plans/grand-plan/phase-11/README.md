...
| **Status** | WP1 and WP2 backend foundations plus WP3 semantic search foundations/benchmark, WP4 hybrid ranking integration, WP5 match-location UI foundation, WP6 erasure/spike foundations, and WP7 UI/i18n foundation started locally: embedding schema upgrade, `IAiProvider` stub, Ollama embedding contract/adapter, vector repository, embedding generation service/worker, semantic read-model progress events, SIMD cosine scoring, deterministic top-K, semantic search service, semantic P95 benchmark, hybrid ranking service, match-location service, semantic search panel metadata/badges, semantic availability indicator, placeholder icon wiring, embedding erasure service, Index Manager erasure confirmation countdown, ANN spike plan, ADR-0006 ANN stub, architecture guards, and focused tests implemented; remaining premium icon replacement, manual accessibility, full regression evidence, and remote CI gates pending |
...
from the Index Manager so users can manage the rebuildable semantic index in
one place.
...
| 2026-06-01 | WP5 backend started: match-location enum, confidence labels, deterministic location derivation from metadata/FTS/semantic hits, enrichment records, and focused tests added | Codex |
| 2026-06-01 | WP6 erasure backend started: transactional embedding-vector deletion, book embedding-status reset, audit event payload, requeue oracle, and focused privacy tests added | Codex |
| 2026-06-01 | WP6 ANN documentation added: sqlite-vec spike plan, `IVectorIndex` proposed contract, trigger criteria, fallback criteria, and ADR-0006 ANN amendment stub committed | Codex |
| 2026-06-01 | WP4/WP5 integration added: semantic search now merges exact and semantic results through hybrid ranking, book signals, match locations, confidence labels, and exact fallback enrichment | Codex |
| 2026-06-01 | WP3 benchmark added: 2,000-book local pre-embedded semantic P95 test with 20 queries and 1,500 ms gate | Codex |
| 2026-06-01 | WP5 UI foundation added: search panel now uses semantic/hybrid results and exposes match-location/confidence metadata in result rows with UI tests | Codex |
| 2026-06-01 | WP6 UI foundation added: Index Manager exposes guarded embedding erasure with confirmation countdown, localized copy, progress state, and UI test coverage | Codex |
| 2026-06-01 | WP7 UI/i18n foundation added: search panel exposes localized semantic availability text, localized confidence labels, focusable match-location badges with accessible names, hover tooltips, and placeholder icon wiring | Codex |
