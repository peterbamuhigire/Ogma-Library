...
| **Status** | WP1 and WP2 backend foundations plus WP3 semantic search foundations/benchmark, WP4 hybrid ranking integration, WP5 match-location UI foundation, and WP6 erasure/spike foundations started locally: embedding schema upgrade, `IAiProvider` stub, Ollama embedding contract/adapter, vector repository, embedding generation service/worker, semantic read-model progress events, SIMD cosine scoring, deterministic top-K, semantic search service, semantic P95 benchmark, hybrid ranking service, match-location service, semantic search panel metadata, embedding erasure service, Index Manager erasure confirmation countdown, ANN spike plan, ADR-0006 ANN stub, architecture guards, and focused tests implemented; remaining WP7 i18n/accessibility polish, manual, and remote CI gates pending |
