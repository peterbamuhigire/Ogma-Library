...
| 2026-06-02 | Implementation | Advanced WP1: added platform-selected Host CA private-key stores with Windows DPAPI, data-directory-scoped macOS Keychain generic-password storage, non-MVP restricted-file fallback, and migration from legacy macOS fallback PFX files into Keychain. |
| 2026-06-02 | Implementation | Advanced WP11: added Phase 16 LAN verification evidence tooling for Windows/macOS runs, including OS/runtime/network capture, focused automated gates, Keychain probing, and same-subnet mDNS/HTTPS observation fields. |
