...
- `LocalCertificateProvisioner` creates a self-signed X.509 root CA, returns a
  ...
  material as a Keychain generic password, and non-MVP platforms retain the
  restricted-file fallback. Existing macOS fallback PFX files are migrated into
  Keychain on first load and then deleted best-effort.
...
| LanHost CA private-key storage | Windows DPAPI store, macOS Keychain generic-password store, and restricted-file fallback for non-MVP platforms are selected behind `IHostCaStore`; macOS legacy fallback PFX files migrate into Keychain on load |
