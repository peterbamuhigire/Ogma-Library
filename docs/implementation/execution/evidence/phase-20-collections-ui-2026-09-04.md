...
The catalogue sidebar now exposes localized create and delete controls for
user collections. Creation trims the entered name, rejects empty input, uses
the catalogue write boundary, reloads the shelf projection, clears the input,
and reports success or failure. Deletion requires a selected collection,
...
Smart-shelf query authoring, rename UX, file/relink actions, and full
...
success status, input reset, selected-collection deletion, filter clearing,
and the corresponding write-boundary calls. The production Release build
also compiles the bound sidebar controls.

## Gate disposition

Closed: basic collection create/delete control sub-gate.

Still open: smart-shelf saved queries, rename UI, file/relink actions,
