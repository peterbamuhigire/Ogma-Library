...
| 17 | Worker Reliability and Observability | IN PROGRESS | Durable leases, heartbeat, follow-up persistence, poison quarantine, resource groups, redacted lifecycle events, OCR/search/embedding lease conversion, safe pending-job cancellation, cancelled metrics, diagnostics export, restart-style load, and Windows/macOS hosted-runner process-kill/restart evidence delivered; cooperative active-handler cancellation, full-app crash, activity-centre, physical reference-machine process, and soak evidence open |
