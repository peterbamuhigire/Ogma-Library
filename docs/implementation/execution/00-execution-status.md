...
| 17 | Worker Reliability and Observability | IN PROGRESS | Durable leases, heartbeat, follow-up persistence, poison quarantine, distinct paused/dead-letter states, resource groups, redacted lifecycle/control events, OCR/search/embedding lease conversion, safe pending cancellation, cooperative OCR page-boundary pause/cancel/resume, metrics, diagnostics, restart-style load, and Windows/macOS hosted process recovery delivered; cooperative control for other active handlers, full-app crash, activity-centre, physical reference-machine process, and soak evidence open |
