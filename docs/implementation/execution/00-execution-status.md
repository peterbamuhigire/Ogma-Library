...
| 16 | Cover, Thumbnail and Spine Assets | IN PROGRESS | Manifest, precedence, projection, output validation, stale-asset GC, bounded lazy variants, allowlisted bounded provider-image client, atomic provider persistence, local detail cover UI, and fail-closed LAN variant authorization delivered; provider source, scale, and physical evidence open |
...
| 18 | Ogma Design System and Application Shell | IN PROGRESS | Design controls, focus and typography plus detail-panel and catalogue-shell localization delivered; application-wide localization, settings, palette, contrast and physical accessibility open |
...
| 21 | Reader Completion and Portability | IN PROGRESS | Core reader portability, bounded import safety, reader import/export UI, and local cache/session/non-crash regression evidence delivered; split/viewer, coordinate fallback, physical crash/accessibility and budget evidence open |
