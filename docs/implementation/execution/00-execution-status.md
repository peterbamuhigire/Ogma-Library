...
1,117 tests (918 core, 41 architecture, 158 UI), with 0 failures and 0 skips.
...
| 16 | Cover, Thumbnail and Spine Assets | IN PROGRESS | Manifest, precedence, projection, output validation, stale-asset GC, bounded lazy variants, bounded embedded-cover acquisition, allowlisted provider-image client wired to enrichment, atomic provider persistence, idempotent spine scheduling on ingest/update, local detail cover UI, and fail-closed LAN variant authorization delivered; large-library budget, physical accessibility, and cross-platform evidence open |
...
| 18 | Ogma Design System and Application Shell | IN PROGRESS | Design controls, focus, typography, application-wide natural-language copy extraction, detail-panel/catalogue-shell/Student Smart Search/Advisor/startup/detail-format/AI accessibility/3D/directory fallback and Host-sharing copy localization, persisted theme/density, command-palette execution, static route inventory, and automated Light/Dark accent contrast matrix delivered; rendered contrast snapshots and physical accessibility open |
